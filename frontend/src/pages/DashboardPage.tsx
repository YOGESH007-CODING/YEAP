...
interface TopicMastery { topicName: string; masteryScore: number; totalAttempts: number }
interface HeatmapResponse { success: boolean; data: { topics: TopicMastery[] } }
interface StreakResponse { success: boolean; data: { currentStreak: number; longestStreak: number; streakSafeToday: boolean; freezesAvailable: number } }
...
export function DashboardPage() {
  ...
  async function loadHeatmap() { try { const res = await api.get<HeatmapResponse>('/api/trackers/topics/heatmap'); setWeakTopics(res.data.topics.slice(0, 5)); } catch { /* no memory data yet */ } }
  ...
  return <div className="mx-auto max-w-screen-xl px-4 py-8">
    {streak && <div className="mb-5 flex items-center justify-between rounded-xl border border-[#5E6AD2]/25 bg-[#5E6AD2]/10 px-4 py-3"><div><span className="text-sm font-semibold text-[#EDEDEF]">🔥 {streak.currentStreak} day streak</span><p className="mt-0.5 text-xs text-[#8A8F98]">{streak.streakSafeToday ? 'Nothing due today — your streak is safe.' : 'Clear today’s queue to keep it going.'}</p></div><span className="font-mono text-[10px] uppercase tracking-widest text-white/40">{streak.freezesAvailable} freeze{streak.freezesAvailable === 1 ? '' : 's'}</span></div>}
