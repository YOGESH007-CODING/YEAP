...
exports.ReviewUseCaseProcessor = void 0;
const SrsEngine_1 = require("../../domain/SrsEngine");
// ─── Use Case ─────────────────────────────────────────────────────────────────
class ReviewUseCaseProcessor {
    constructor(deps) {
     ...
     */
    async execute(userId, dto, verifyWithLeetCode = false) {
        ...
        if (verifyWithLeetCode) {
            ...
            const verified = await this.leetCodeClient.verifyUserSubmission(user.leetcodeUsername, problem.slug);
            if (!verified) {
        ...
        const now = new Date();
        const updatedProgress = await this.progressRepo.atomicFindAndUpdate(userId, dto.problemId, (current) => {
            const currentState = {
                repetitions: current.repetitions,
                easinessFactor: current.easinessFactor,
                intervalDays: current.intervalDays,
            };
            const result = SrsEngine_1.SrsEngine.calculateNextReview(currentState, dto.qualityScore);
            return {
...
exports.ReviewUseCaseProcessor = ReviewUseCaseProcessor;
//# sourceMappingURL=ReviewUseCaseProcessor.js.map