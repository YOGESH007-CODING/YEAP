...
export class ResendNotificationProvider implements INotificationProvider {
  ...
  private difficultyBadge(difficulty: string): string {
    const normalized = difficulty.toUpperCase();
    const [fg, bg, edge]: readonly [string, string, string] =
      normalized === 'EASY' ? [T.easy, T.easyTint, T.easyEdge]
  ...
  private sectionLabel(label: string, count: number): string {
    return `
      <tr>
  ...
  private statCell(label: string, value: string, color: string, first: boolean): string {
    return `
  ...
  private reviewTable(items: ReviewItem[]): string {
    const th = (text: string, align: string) =>
      `<th align="${align}" style="padding:9px 12px;font-family:${T.mono};font-size:9px;font-weight:500;letter-spacing:0.12em;text-transform:uppercase;color:${T.textTertiary};text-align:${align};">${text}</th>`;
